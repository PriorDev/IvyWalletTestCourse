...
class SyncExchangeRatesActTest {
    ...
    private lateinit var exchangeRateDaoFake: ExhangeRateDaoFake

    @BeforeEach
    fun setUp() {
        exchangeProviderFake = RemoteExchangeProviderFake()
        exchangeRateDaoFake = ExhangeRateDaoFake()
        syncExchangeRatesAct = SyncExchangeRatesAct(exchangeProviderFake, exchangeRateDaoFake)
    }
    ...
    @Test
    fun `Test sync exchange rate, valid values saved`() = runBlocking<Unit> {
        syncExchangeRatesAct("USD")
