...
class ExchangeRatesFlowTest {
    ...
    fun setUp() {
        ...
        exchangeRatesFlow = ExchangeRatesFlow(
            baseCurrencyFlow, exchangeRateDao, exchangeRateOverrideDao, testDispatchers
        )
    }

    @Test
    fun `Test echange rates flow emission`() = runTest {
        ...
        exchangeRatesFlow().test {
            ...
            exchangeRateOverrideDao.save(exchangeRatesOverride)

            val rates1 = awaitItem()
            assertThat(rates1.rates).hasSize(exchangeRates.size)
            ...
            assertThat(rates2.rates).hasSize(exchangeRates.size)
